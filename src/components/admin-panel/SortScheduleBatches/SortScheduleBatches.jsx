        ...
        try {
            ...
            if (res.isSuccess == true) {
                ...
                for(let i = 0 ; i < res.scheduleBatches.length; i++){
                 if(res.scheduleBatches[i].notScheduleYet == false && res.scheduleBatches[i].everyWeekend == false){
                    temp = res.scheduleBatches[i];
                    temp.batchName = formatDate(res.scheduleBatches[i].eventStartDate)+' - '+formatDate(res.scheduleBatches[i].eventEndDate);
                    tempBatch.push(temp);
                 }
                }
